...
print(Fore.YELLOW + Style.BRIGHT + '-' * 37)
...
def main_menu():
    ...
    user_input = 0
    while user_input != 3:
        user_input = int(input(Fore.RED + '''What would you like to do?\n
        1. Login
...
def login_menu(username):
    ...
    user_input = 0
    while user_input != 5:
        user_input = int(input(Fore.BLUE + '''\nChoose an option:\n
        1. See my portfolio
        2. Stock search
        3. Buy stocks
        4. Sell stocks
...
def buy_stock(username):
    ...
    last_price, total_price, balance, response = controller.buy_stock(symbol, qty, username)
    if response == 400:
        ...
        print('You have successfully purchased {} shares of {} at {}.'.format(qty, symbol, last_price))
...
def sell_stock():
    ...
    last_price, total_price, balance, response = controller.sell_stock(symbol, qty, username)
    if response == 400:
        ...
        print('You have successfully sold {} shares of {} at {}.'.format(qty, symbol, last_price))
        print('For a total of ${}'.format(total_price))
        print('Your balance is now ${}.'.format(balance))
